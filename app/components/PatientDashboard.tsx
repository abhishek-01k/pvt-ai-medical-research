...
export const PatientDashboard: FC = () => {
  ...
  const loadPatientAttestations = async () => {
    ...
    try {
      ...
      const patientAttestations = await service.getPatientAttestations(walletClient.account.address);
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        {!walletClient ? (
          ...
          >
            {showResearchForm ? 'Hide Research Form' : 'Participate in Research'}
          </button>
        )}
      </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {attestations.map((attestation) => (
              <div key={attestation.id} className="border rounded-lg p-4 bg-white shadow">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-gray-500">Date</p>
                    <p>{new Date(attestation.timestamp * 1000).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Status</p>
                    <p>{attestation.isVerified ? 
